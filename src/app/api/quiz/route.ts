...
export async function POST(req: NextRequest) {
  ...
  const result = await generateQuizQuestions(module, customInstructions ?? "", lang);

  if (session.missionaryId && !result.error) {
    await logActivity(session.missionaryId, "assistant", `[Quiz generated: ${result.module_title}]`, "quiz_master", module);
  }
