...
export default function MentorLoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error ?? "Invalid mentor access code");
      return;
    }
    router.push("/mentor");
    router.refresh();
    ...
    <main className="flex min-h-screen items-center justify-center px-4">
      ...
      >
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-2 font-serif text-xl tracking-wide text-harvest-gold">Mentor Dashboard · Panel del Mentor</h1>
        ...
        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-harvest-textDim">
          Mentor access code · Código de acceso del mentor
        </label>
        <input
          autoFocus
          type="password"
        ...
        >
          {loading ? "Checking…" : "Access dashboard · Acceder al panel"}
        </button>

        <p className="mt-4 text-center text-xs text-harvest-textDim">
          <a href="/login" className="text-harvest-gold hover:underline">
            ← Back · Volver
          </a>
        </p>
      </form>
